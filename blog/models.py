...
class Post(db.Model):
    ...
    created_at = db.Column(
        db.DateTime, default=datetime.datetime.now(datetime.timezone.utc), index=True
    )
    updated_at = db.Column(
        db.DateTime,
        default=datetime.datetime.now(datetime.timezone.utc),
        onupdate=datetime.datetime.now(datetime.timezone.utc),  # 更新时自动修改
    )
...
class Product(db.Model):
    ...
    def price_history(self, days=30):
        ...
        return (
            self.price_records.filter(PriceRecord.recorded_at >= since)
...
class ProductSource(db.Model):
    ...
    latest_price = db.Column(db.Numeric(10, 2), default=0)
...
class BiliWatchedVideo(db.Model):
    ...
    added_at = db.Column(db.DateTime, default=datetime.datetime.now(datetime.timezone.utc))
