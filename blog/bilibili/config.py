...
# 核心原则：低频 + 随机抖动 + 大间隔，模拟人类浏览行为
# 可通过环境变量覆盖（如 BILI_INTERVAL=20 将间隔调至 20s）
REQUEST_INTERVAL = float(os.environ.get('BILI_INTERVAL', '15.0'))
# 翻页间隔基础值（秒）— 最终值 = 基础值 + random(0, JITTER)
REQUEST_INTERVAL_JITTER = float(os.environ.get('BILI_JITTER', '10.0'))
# 翻页间隔随机抖动范围（秒），避免固定频率被检测
