...
from blog.core.models import BiliDynamic, BiliSubscription, BiliUp, BiliUpHistory, BiliVideo, BiliVideoHistory, WordCloudData, db
...
def up_videos(up_id):
    ...
    try:
        ...
        wc_months = (
            WordCloudData.query
            .filter_by(post_id=None, source='bili')
            .filter(WordCloudData.period.like(escape_like(f'up_{up_id}_') + '%', escape='\\'))
            .order_by(WordCloudData.period)
            .all()
        )
        for wcm in wc_months:
...
@bili_public_bp.route('/up/<int:up_id>/dynamics')
def up_dynamics(up_id):
    """公开的 UP 主动态列表页

    展示动态分页列表（每页 20 条，按发布时间倒序），
    支持视频投稿/图文/文字/转发四种类型。
    ...
    Returns:
        HTML 页面，渲染 bilibili_up_dynamics.html
    """
    up = BiliUp.query.get_or_404(up_id)
    page = request.args.get('page', 1, type=int)
    per_page = 20
    pagination = (
        BiliDynamic.query.filter_by(up_id=up_id)
        .order_by(BiliDynamic.pub_datetime.desc())
        .paginate(page=page, per_page=per_page, error_out=False)
    )
    return render_template(
        'bilibili_up_dynamics.html',
        up=up,
        pagination=pagination,
    )
